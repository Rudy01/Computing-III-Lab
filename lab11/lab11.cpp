...
{
	if (lhs.getIDNumber() < rhs.getIDNumber()) {
		return true;
	}
	return false;
}
bool comp_lastname(const UMLPerson& lhs, const UMLPerson& rhs)
{
	if (lhs < rhs) {
		return true;
	}
...
//	createUMLPeopleDB(); // not needed for Lab 11 - output is already in file lab11_input.txt
	...
	// *** You write it!
	stable_sort(upDB.begin(), upDB.end(), comp_lastname);
	...
	if (find(upDB.begin(), upDB.end(), keyID) < upDB.end()) {
		cout << endl << "found key ID" << endl;
	}

	// Trying out STL algorithm minmax_element...
	cout << "std::minmax_element in upDB using default comparison (by last name):\n";
	auto result2 = std::minmax_element(upDB.begin(), upDB.end(), comp_lastname);

	// print result:
	cout << "min is " << *result2.first;
	cout << "max is " << *result2.second;
	cout << "std::minmax_element in upDB using Compare function compare_IDs:\n";
	// *** You write it!
	auto result = std::minmax_element(upDB.begin(), upDB.end(), compare_IDs);
